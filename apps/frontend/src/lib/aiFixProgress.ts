...
/**
 * Does a stored CI diagnosis predate the PR's current head? ONE predicate, read twice on the
 * same card — by the "out of date" chip and by the "Fix it" gate. They must never disagree: a
 * chip saying the analysis is old beside a button that seeds an agent with it is the defect.
 *
 * ⚠ A stored analysis with NO head sha is NOT stale. It predates the column and cannot be
 * disproved; claiming staleness we cannot show is the same overreach in the other direction.
 * `apps/backend`'s twin (the plugin's `ciSeedDecision`) reads the null the same way.
 */
export function ciAnalysisStale(
  ...
  return analysis != null && storedHeadSha != null && storedHeadSha !== prHeadSha;
}
