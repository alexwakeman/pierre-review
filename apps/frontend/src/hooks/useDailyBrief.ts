...
// The daily brief (plan P3.1/P3.3) — CORE/free counts for the Welcome-back banner and the Workspace
// badges (`useMyTurnByWorkspace`). (The Feed's strip that drew one line per figure is deleted.)
// Always asks for the roll-up too: the server returns it only when the account has other
// workspaces, each ROLL-UP line riding the fold's server-side 5-min TTL cache, so the extra flag
// is one bounded loop of cache hits — not a second request a caller would have to orchestrate. (The ACTIVE workspace's
...
export function useDailyBrief(workspaceId: number | null) {
  return useQuery<DailyBriefResponse>({
    ...
    // `myTurn` IS the my_turn card count GET /api/attention serves, and the banner's/badges'
    // figure is the board's card count — so two different lifetimes on the two keys is two snapshots of one fold, and
    ...
    // The banner's/badges' figure is the board's card count, so a focus that refreshed one and
    // not the other would put a stale count beside a fresh list.
