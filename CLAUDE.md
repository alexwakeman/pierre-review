...
(`index.ts` runs core `runMigrations()` before `bindProPlugin()` → `runPluginMigrations`). Plugin
`0019` **only ever UPDATEs, never INSERTs** (fabricating a row would invent
`automated`/`confidence`/`source` judgements nobody made, and a fabricated `source='manual'` row
would permanently shadow auto-detection), targets `team_id = 0` only, is guarded on
`cost_monthly_cents IS NULL` (idempotent, and can never clobber a deliberate `0`), and does NOT drop
the blob — unmatched logins keep driving ROI through the read-time fallback. **Two dialect
divergences on hand-corrupted input, both verified by replay from empty and neither of them lossy:**
(a) a 3-decimal legacy value rounds differently — `1.005` → sqlite `100`, pg `101` (every 2-decimal
value matches); (b) a corrupt blob that still LOOKS like an array (`'[oops'`) passes pg's `~ '^\s*\['`
guard, so the `::jsonb` cast raises, the `DO` block downgrades it to a WARNING and the whole
statement rolls back — **no tenant's costs migrate**, while sqlite's `json_valid()` guard degrades it
to `'[]'` and every other tenant migrates fine. The migration is still recorded as applied either
way, so it never retries; the skipped values are un-migrated, not lost. Plugin migration `0017` adds `pr_comment_annotations` (+ backfills
...
or it SILENTLY skips** — `0038`–`0043` and pg `0025`–`0030` are registered (plugin migrations are
